let mazeHard = document.querySelector('.mazeHard');
...
class MazeHard {
    ...
    draw() {
        mazeHard.width = this.size;
        mazeHard.height = this.size;
        mazeHard.style.background = "black";
...
let newMazeHard = new MazeHard(640, 20, 20);
newMazeHard.setup()
newMazeHard.draw()
...
function isTileBlocked(tileX, tileY, direction) {
    ...
    if (row < 0 || row >= newMazeHard.rows || col < 0 || col >= newMazeHard.columns) return true;

    const cell = newMazeHard.grid[row][col];
