let mazeEasy = document.querySelector('.mazeEasy');
let body = document.querySelector('.body');
let ctx = mazeEasy.getContext('2d');
...
class MazeEasy {
    ...
    draw() {
        mazeEasy.width = this.size;
        mazeEasy.height = this.size;
        mazeEasy.style.background = "black";
...
let newMazeEasy = new MazeEasy(160, 5, 5);
newMazeEasy.setup()
newMazeEasy.draw()
...
function isTileBlocked(tileX, tileY, direction) {
    ...
    if (row < 0 || row >= newMazeEasy.rows || col < 0 || col >= newMazeEasy.columns) return true;

    const cell = newMazeEasy.grid[row][col];
